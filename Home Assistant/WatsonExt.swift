...
extension ViewController {
  ...
  func setupWatson() {
    ...
    conversation?.message(workspaceID: Credentials.ConversationWorkspaceID, failure: failure) {
      response in
  ...
  */
  ...
  func conversationRequestResponse(_ text: String) {
    ...
    self.conversation?.message(withWorkspace: Credentials.ConversationWorkspaceID, request: request, failure: failure) {
                                response in
  ...
  func issueCommand(intents: [Intent], entities: [Entity]) {
    ...
    for intent in intents {
        print("intent: \(intent.intentName), confidence: \(intent.confidence) ")
    }
